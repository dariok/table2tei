		...
		<xsl:if test="ancestor::tei:table/tei:row[1]/tei:cell[$pos]/text()">
			<xsl:choose>
				<xsl:when test="ancestor::tei:table/tei:row[1]/tei:cell[$pos + 1]/text()">
					<w:r>
						...
						<w:t xml:space="preserve"><xsl:value-of
							select="normalize-space(ancestor::tei:table/tei:row[1]/tei:cell[$pos])"/><xsl:text>: </xsl:text></w:t>
					</w:r>
					<w:r>
				...
				<xsl:otherwise>
					...
					<w:r>
						<w:t xml:space="preserve"><xsl:value-of select="normalize-space()" /><xsl:text> – </xsl:text><xsl:value-of
							select="normalize-space(following-sibling::tei:cell[1])" /></w:t>
					</w:r>
					<w:r>
						<w:br/>
					</w:r>
