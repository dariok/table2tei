...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema"
   ...
   xmlns:rel="http://schemas.openxmlformats.org/package/2006/relationships" xmlns="http://www.tei-c.org/ns/1.0"
      ...
      <TEI version="5.0">
         <teiHeader>
            <fileDesc>
               ...
               <sourceDesc>
                  <p>Created from <xsl:value-of select="$filename"/> by table2TEI, ms2TEI.xsl on <xsl:value-of
   ...
   <xsl:template match="sheet:row[sheet:c]">
      ...
      <row>
         ...
         <xsl:if test="
               (string-length($free) &gt; 0 and following-sibling::sheet:row/sheet:c[@r = $free])
         ...
         <xsl:choose>
            <xsl:when test="ancestor::sheet:worksheet/sheet:hyperlinks/sheet:hyperlink[@ref = $r]">
               <xsl:variable name="hyperlink"
                  select="ancestor::sheet:worksheet/sheet:hyperlinks/sheet:hyperlink[@ref = $r]"/>
